...
class BBRBMTrainingParams:
    def __init__(
        self, n_components=4, algorithm='cd', str_cost='energy', 
...
class BBRBM:
    ...
    def reconstruct(self, xs):
        ...
        """
        w, b, _ = _get_params(self.params_model)
        hs      = self.transform(xs)
        vs      = _mean_vs_given_hs_np(hs, w, b)
    ...
    def transform(self, xs):
        ...
        """
        w, _, c = _get_params(self.params_model)
        vs      = xs
        hs      = _mean_hs_given_vs_np(vs, w, c)
...
def _show_info(get_info, i, t):
    ...
    mean_w, mean_b, mean_c = info[2], info[3], info[4]
    max_w,  max_b,  max_c  = info[5], info[6], info[7]

    print('Iteration %d, Error (train, test) = (%f %f), time = %.2fs'
          % (i, err_tr, err_vl, t))
    print('Means (w, b, c) = (%+1.2e, %+1.2e, %+1.2e)'
          % (mean_w, mean_b, mean_c))
    print('Maxs  (w, b, c) = (%+1.2e, %+1.2e, %+1.2e)'
          % (max_w, max_b, max_c))
...
def _get_params(params):
    return (params['w'], params['b'], params['c'])

def _get_params_np(params):
    return (params['w'], params['b'], params['c'])

def _get_diffs(diffs):
    return (diffs['dw'], diffs['db'], diffs['dc'])
...
def _init_theano(xs_tr_np, xs_vl_np, params_np, params_tr):
    ...
    poss = _get_poss(rng, v0s, params)
    negs = _get_negs(rng, poss, params)
    ...
    recon_err_tr = _get_recon_err(xs_tr, params)
    recon_err_vl = _get_recon_err(xs_vl, params)
...
def _get_updates_init_particles(rng, particles, v0s, params, params_tr):
    ...
    hs = _sample_hs_given_vs(rng, v0s, w, c)
...
def _get_cost(poss, negs, params, params_tr):
    ...
    e_pos        = _energy(vs_pos, hs_pos, w, b, c)
    e_neg        = _energy(vs_neg, hs_neg, w, b, c)
    pw           = _penalty_weight(w, lmd_w)
    ph, const_ph = _penalty_hidden_act(vs_pos, w, c, lmd_h)
...
def _penalty_hidden_act(vs, w, c, lmd_h):
    if lmd_h == 0.0:
        return scipy.float32(0.0), []
    else:
        vw = T.dot(vs, w)

        return lmd_h * T.sum(T.abs_(T.nnet.sigmoid(vw + c))), [vw]

def _energy(vs, hs, w, b, c):
    bsize = vs.shape[0]
    e1    = T.mean(T.sum(b * vs, axis=1))
    e2    = T.mean(T.sum(c * hs, axis=1))
    e3    = T.sum(T.dot(vs.T, hs) * w) / bsize
...
def _get_recon_err(vs, params):
    if vs is None:
        err = shared(0.0)
    else:
        # Reconstruction
        w, b, c = _get_params(params)
        hs      = _mean_hs_given_vs(vs, w, c)
        vs_rec  = _mean_vs_given_hs(hs, w, b)
...
def _get_poss(rng, v0s, params):
    w, b, c = _get_params(params)
    vs      = v0s
    hs      = _sample_hs_given_vs(rng, vs, w, c)
    
    return {'vs': vs, 'hs': hs}

def _get_negs(rng, particles, params):
    w, b, c = _get_params(params)
    _hs     = particles['hs']

    def _gibbs_hvh(hs0):
        vs1 = _sample_vs_given_hs(rng, hs0, w, b)
        hs1 = _sample_hs_given_vs(rng, vs1, w, c)
...
def _sample_hs_given_vs(rng, vs, w, c):
    ps = _mean_hs_given_vs(vs, w, c)
    hs = rng.binomial(size=ps.shape, n=1, p=ps, dtype=cnf.floatX)
    return hs

def _sample_vs_given_hs(rng, hs, w, b):
    ps = _mean_vs_given_hs(hs, w, b)
    vs = rng.binomial(size=ps.shape, n=1, p=ps, dtype=cnf.floatX)
    return vs

def _sample_hs_given_vs_np(vs, w, c):
    ps = _mean_hs_given_vs_np(vs, w, c)
    hs = np.random.binomial(size=ps.shape, n=1, p=ps)
    return hs

def _sample_vs_given_hs_np(hs, w, z, b):
    ps = _mean_vs_given_hs_np(hs, w, b)
    vs = np.random.binomial(size=ps.shape, n=1, p=ps)
    return vs
...
def _mean_hs_given_vs(vs, w, c):
    return T.nnet.sigmoid(T.dot(vs, w) + c)

def _mean_vs_given_hs(hs, w, b):
    return T.nnet.sigmoid(T.dot(hs, w.T) + b)

def _mean_hs_given_vs_np(vs, w, c):
    return _sigmoid_np(np.dot(vs, w) + c)

def _mean_vs_given_hs_np(hs, w, b):
    return _sigmoid_np(np.dot(hs, w.T) + b)
